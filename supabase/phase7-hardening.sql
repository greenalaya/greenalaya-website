...
--
-- Rate limiting: contact_submissions allows public inserts
-- (`to anon, authenticated with check (true)`) directly via the anon key
-- over PostgREST, bypassing any rate limiting implemented in the Next.js
-- server actions. This trigger enforces a per-IP limit at the database
-- layer, which is the only layer every insert must pass through.
--
